...
async function authentication(req, res, next) {
    try {
        ...
        let user = await User.findByPk(payload.id)//mencari payload.id  //redis
...
async function authenticationCus(req, res, next) {
    try {
        ...
        //4.cari user dari token yang sudah di verify => kalau user tidak ada lempar error
        let user = await Customer.findByPk(payload.id)//mencari payload.id  //redis
        if (!user) throw ({ name: "InvalidToken" })
...
module.exports = { authentication, authenticationCus }